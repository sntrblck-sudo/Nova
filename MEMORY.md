...
- [2026-04-02 12:30:00] [nova,x402,collector] EIP-3009 collector built: collect_auths.cjs. Reads pending_auths.jsonl, executes transferWithAuthorization on USDC. AuthorizationUsed event verified on-chain — USDC EIP-3009 pull-payment path confirmed functional. Nonce: USDC uses sequential nonce (checked via nonces() view). Auth collection path exists, collector script ready to cron.
- [2026-04-02 13:00:00] [nova,x402,collector,note] USDC transferWithAuthorization verified working via AuthorizationUsed + Transfer events (tx: 0x2fc98b92... nonce=0 consumed). Self-transfer test proved the mechanism. Pull-payment collection confirmed.
- [2026-04-02 12:15:00] [nova,x402,api,deployed] Nova's x402 API server deployed LIVE on Base Mainnet via localtunnel. URL: https://nova-api-live.loca.lt. Server: cdp-nova/nova-api-server/index.cjs (Express + viem). Port 3001. Price: $0.10 USDC per query via x402 EIP-3009. Endpoints: / (manifest), /health, /balance/:addr (paid), /query (paid). Supports both Authorization:FUTUREU and X-PAYMENT (x402 v1) headers. x402 payment flow verified end-to-end: payment auth signed → server verifies → 200 + query data returned. USDC actual transfer is pull-based (EIP-3009), not push-based. Current tunnel URL in nova-api-server/tunnel_url.txt.
