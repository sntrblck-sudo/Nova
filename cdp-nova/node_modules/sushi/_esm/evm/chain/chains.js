...
import { apeChain as apeViem, arbitrumNova as arbitrumNovaViem, arbitrumSepolia as arbitrumSepoliaViem, arbitrum as arbitrumViem, avalanche as avalancheViem, base as baseViem, berachain as berachainViem, blast as blastViem, boba as bobaViem, bsc as bscViem, bitTorrent as bttcViem, celo as celoViem, coreDao as coreViem, cronos as cronosViem, mainnet as ethereumViem, fantom as fantomViem, filecoin as filecoinViem, gnosis as gnosisViem, haqqMainnet as haqqViem, harmonyOne as harmonyViem, hemi as hemiViem, kava as kavaViem, linea as lineaViem, manta as mantaViem, mantle as mantleViem, metis as metisViem, mode as modeViem, optimism as optimismViem, polygon as polygonViem, polygonZkEvm as polygonZkevmViem, rootstock as rootstockViem, scroll as scrollViem, sepolia as sepoliaViem, skaleEuropa as skaleEuropaViem, sonic as sonicViem, taiko as taikoViem, thunderCore as thundercoreViem, zetachain as zetachainViem, zkLinkNova as zklinkViem, zksync as zksyncViem, } from 'viem/chains';
...
// EvmChainId
export const evmChainIds = /* @__PURE__ */ evmChains.map((chain) => chain.chainId);
const chainIdByKey = Object.freeze(evmChains.reduce((acc, { key, chainId }) => {
    acc[key.toUpperCase().replace(/-/g, '_')] = chainId;
    return acc;
}, {}));
export const EvmChainId = chainIdByKey;
const chainById = Object.freeze(Object.fromEntries(evmChains.map((c) => [c.chainId, c])));
...
export function isEvmMainnetChainId(chainId) {
    return chainById[chainId]?.netType === 'mainnet';
}
export function isEvmTestnetChainId(chainId) {
    return chainById[chainId]?.netType === 'testnet';
}
export function isEvmChainKey(key) {
    return evmChains.some((chain) => chain.key === key);
}
export function getEvmChainByKey(key) {
    return evmChains.find((chain) => chain.key === key);
}
