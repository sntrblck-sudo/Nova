...
// }
export const formatNumber = (value, maxDecimalPlaces = 2) => {
    value = value ?? 0;
    ...
    return `${negative ? '-' : ''}${formatValueWithSuffix(value, maxDecimalPlaces)}`;
};
const formatValueWithSuffix = (value, maxDecimalPlaces) => {
    ...
    const decimalCount = Math.min(value.toString().split('.')[1]?.length || 0, maxDecimalPlaces);
 ...
 */
export function withoutScientificNotation(value) {
    if (!value.includes('e'))
        return value;
 ...
 * @returns
 */
export function numberToFixed(num, args) {
    ...
    if ('maxFixed' in args) {
        ...
        let end = str.length;
        while (end && str.charCodeAt(end - 1) === 48)
            --end;
        if (end && str.charCodeAt(end - 1) === 46)
            --end;
        return `${str.slice(0, end)}`;
    }
    throw new Error('Invalid arguments for numberToFixed');
}
function stringToFixed(_str, args) {
...
function roundString(original, formatted) {
    ...
    if (!Number.isNaN(nextDigit) && nextDigit >= '5'.charCodeAt(0)) {
        ...
        return `${formatted.substring(0, formatted.length - 1)}${String.fromCharCode(lastDigit() + 1)}`.padEnd(decimalIndex, '0');
    }
    return formatted;
}
